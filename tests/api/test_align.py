...
def test_align_inside_block_both_targets(client: TestClient) -> None:
    body = client.get(
        "/api/align", params={"genome_id": "A", "seq": "chr1", "pos": 500}
    ).json()
...
def test_align_before_all_blocks(client: TestClient) -> None:
    ...
    body = client.get(
        "/api/align", params={"genome_id": "A", "seq": "chr1", "pos": 50}
    ).json()
    m_b = _mapping(body, "B")
...
def test_align_after_all_blocks(client: TestClient) -> None:
    ...
    body = client.get(
        "/api/align", params={"genome_id": "A", "seq": "chr1", "pos": 1500}
    ).json()
    m_b = _mapping(body, "B")
...
def test_align_unknown_genome_404(client: TestClient) -> None:
    r = client.get(
        "/api/align", params={"genome_id": "NOPE", "seq": "chr1", "pos": 100}
    )
...
def test_align_unknown_seq_404(client: TestClient) -> None:
    r = client.get(
        "/api/align", params={"genome_id": "A", "seq": "ghost", "pos": 100}
    )
    assert r.status_code == 404
...
def test_align_bp_to_pixel_roundtrip(client: TestClient) -> None:
    ...
    r2 = client.get(
        "/api/align", params={"genome_id": "B", "seq": "chr1", "pos": pos_on_b}
    ).json()
